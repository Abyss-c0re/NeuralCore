...
class LLMClient:
    ...
    async def agent_stream(
            ...
            while iteration < max_iterations:
                ...
                # ← NEW: Early stop check at start of every iteration
                if self._current_stop_event is not None and self._current_stop_event.is_set():
                    logger.info("Agent stopped by user before starting iteration")
                ...
                # ← NEW: Stop check after LLM response (before deciding final answer or tools)
                if self._current_stop_event is not None and self._current_stop_event.is_set():
                    logger.info("Agent stopped by user after LLM response")
                    yield ("cancelled", "Stop requested")
                    return
                ...
                for call in tool_calls:
                    ...
                    # ← NEW: Stop check after every tool (prevents starting more tools)
                    if self._current_stop_event is not None and self._current_stop_event.is_set():
                        logger.info(f"Agent stopped by user after tool {name}")
                        yield ("cancelled", "Stop requested")
                        return
